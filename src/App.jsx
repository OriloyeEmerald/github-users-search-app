import { useState } from 'react'
import './App.css'
import Navbar from './components/navbar'
import Profile from './components/profile'
import Searchbar from './components/searchbar'
import axios from 'axios';


const API_URL = `https://api.github.com/users/`
function App() {
const [toggleState, setToggleState] = useState(true)
const [name, setName] = useState('')
const [username, setUsername] = useState('@babayaga')
const [title, setTitle] = useState('John Wick')
const [date, setDate] = useState('Jan 01, 2001')
const [profileImg, setProfileImg] = useState('data:image/png;base64,')
const [bio, setBio] = useState('Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Donec odio.ipsum dolor sit amet ')
const [repo, setRepo] = useState(0)
const [followers, setFollowers] = useState(0)
const [following, setFollowing] = useState(0)
const [location, setLocation] = useState(`John Wick's House`)
const [url, setUrl] = useState('https://github.blog')
const [twitter, setTwitter] = useState('Not Available')
const [company, setCompany] = useState('Hidden Leaf')
const [isLoading, setIsLoading] = useState(false);

const handleToggleState = () => {
  setToggleState(!toggleState);

  document.body.className = toggleState &&'light-theme';
}

 const fetchUsers = async () => {
       setIsLoading(true);
       try {
        const response = await axios.get(`${API_URL}${name}`);
        const result = response;
        setUsername('@' + name)
        setTitle(result.data.name)
        const updatedDate = result.data.updated_at;
        const newDate = new Date(updatedDate);
        
        const options = { day: '2-digit', month: 'long', year: 'numeric' };
        const formattedDate = newDate.toLocaleDateString('en-US', options);
        setDate(formattedDate)
       
        setProfileImg(result.data.avatar_url)
        setBio(result.data.bio)
        setRepo(result.data.public_repos)
        setFollowers(result.data.followers)
        setFollowing(result.data.following)
        {!result.data.location && (
          <p>location unavailable</p>
        )}
        setLocation(result.data.location
          )
        setUrl(result.data.html_url
          )
        setTwitter(result.data.twitter_username)
        setCompany(result.data.company
          )
          setName('')
       } catch (error) {
        if (error.result && error.result.status === 404) {
          alert('User not found:', error);
          setName('')
        } else {
          alert('Error:', error);
          setName('')
        }
       } finally {
        setIsLoading(false)
       }

 }

  return (
    <>
      <Navbar 
      toggleState={toggleState} handleToggleState={handleToggleState}
      />
      <Searchbar 
        toggleState={toggleState}
        name={name} 
        setName={setName}
        fetchUsers={fetchUsers}
      />
      <Profile 
      username={username}
      title={title}
      date={date}
      profileImg={profileImg}
      bio={bio}
      repo={repo}
      followers={followers}
      following={following}
      location={location}
      url={url}
      twitter={twitter}
      company={company}
      toggleState={toggleState}
      isLoading={isLoading}
      />

    </>
  )
}

export default App